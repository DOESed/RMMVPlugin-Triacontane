...
/*:
 * @plugindesc 動態文字圖片生成插件
 * @author トリアコンタン
 *
 * @help 提供動態生成具有指定文字的圖片命令。
 * 各種控制字元（\v[n]等）也可用於文字，並且當控制字元顯示的變量值發生變化時，
 * 您可以實時更新圖像的內容。
 *
 * 按以下步驟顯示動態文字圖片
 ...
 *  文字顯示還支持控制字元，如變量和角色的顯示。
 *
 *  D_TEXT_SETTING ALIGN [對齊] : 對齊設定（左對齊、中間對齊、右對齊）
 *  0:左對齊 1:中間對齊 2:右對齊
 *
 *  例子：D_TEXT_SETTING ALIGN 0
 *       D_TEXT_SETTING ALIGN CENTER
 *
 *  D_TEXT_SETTING BG_COLOR [背景色] : 背景顏色設置（與CSS顏色規格相同的格式）
 *
 *  例子：D_TEXT_SETTING BG_COLOR black
 *      D_TEXT_SETTING BG_COLOR #336699
 *      D_TEXT_SETTING BG_COLOR rgba(255,255,255,0.5)
 *
 *  D_TEXT_SETTING REAL_TIME ON : 使用控制字元顯示變量的實時顯示
 *
 *  例子：D_TEXT_SETTING REAL_TIME ON
 *
 *  啟用實時顯示後，當顯示圖像後變量值發生變化時，圖像內容也會自動更新。
 *
 *  D_TEXT_SETTING WINDOW ON : 顯示窗口背景
 *  例子：D_TEXT_SETTING WINDOW ON
 *
 *  D_TEXT_SETTING FONT [字體名] : 更改描繪字體
